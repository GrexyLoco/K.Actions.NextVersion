...
BeforeAll {
	# Module unter Test importieren
	$script:modulePath = Join-Path (Split-Path $PSScriptRoot -Parent) 'NextVersion' 'NextVersion.psd1'
...
Describe "🔍 K.Actions.NextVersion Tests" {
	...
	Context "📁 Helper Function - Bump-Version" {
		...
		It "Should correctly increment major version" {
			$result = Update-Version -currentVersion "1.2.3" -bumpType "major"
			$result | Should -Be "2.0.0"
		}
        
		It "Should correctly increment minor version" {
			$result = Update-Version -currentVersion "1.2.3" -bumpType "minor"
			$result | Should -Be "1.3.0"
		}
        
		It "Should correctly increment patch version" {
			$result = Update-Version -currentVersion "1.2.3" -bumpType "patch"
			$result | Should -Be "1.2.4"
		}
        
		It "Should handle zero versions" {
			$result = Update-Version -currentVersion "0.0.0" -bumpType "patch"
			$result | Should -Be "0.0.1"
		}
        
		It "Should handle large version numbers" {
			$result = Update-Version -currentVersion "99.99.99" -bumpType "major"
			$result | Should -Be "100.0.0"
		}
        
		It "Should add alpha suffix to major version" {
			$result = Update-Version -currentVersion "1.2.3" -bumpType "major" -suffix "alpha"
			$result | Should -Be "2.0.0-alpha"
		}
        
		It "Should add beta suffix to minor version" {
			$result = Update-Version -currentVersion "1.2.3" -bumpType "minor" -suffix "beta"
			$result | Should -Be "1.3.0-beta"
		}
        
		It "Should add alpha suffix to patch version" {
			$result = Update-Version -currentVersion "1.2.3" -bumpType "patch" -suffix "alpha"
			$result | Should -Be "1.2.4-alpha"
		}
        
		It "Should handle empty suffix parameter" {
			$result = Update-Version -currentVersion "1.2.3" -bumpType "minor" -suffix ""
			$result | Should -Be "1.3.0"
		}
...
Describe "🚀 Integration Tests - Release Based Versioning" {
	...
	Context "🔄 Main Script Execution" {
		...
		It "Should handle target branch validation (not on target branch)" {
			...
			$result = Get-NextVersion -ManifestPath $script:testManifestPath -BranchName "feature/test" -TargetBranch "main"
		...
		It "Should handle manifest path auto-discovery" {
			...
			try {
				$result = Get-NextVersion -ManifestPath "" -BranchName "master" -TargetBranch "master"
		...
		It "Should return error object for non-existent manifest path" {
			$result = Get-NextVersion -ManifestPath "C:\NonExistent\Manifest.psd1" -BranchName "main" -TargetBranch "main" 2>$null
		...
		It "Should return error object when no manifests found during auto-discovery" {
			...
			try {
				$result = Get-NextVersion -ManifestPath "" -BranchName "main" -TargetBranch "main" 2>$null
	...
	Context "⚠️ Error Handling" {
		...
		It "Should handle invalid manifest gracefully" {
			...
			try {
				$result = Get-NextVersion -ManifestPath $invalidManifest -BranchName "main" -TargetBranch "main" 2>$null
...
"@
			...
			try {
				$result = Get-NextVersion -ManifestPath $noVersionManifest -BranchName "main" -TargetBranch "main" 2>$null
		...
		It "Should return object with correct properties on success" {
			$result = Get-NextVersion -ManifestPath $script:testManifestPath -BranchName "master" -TargetBranch "master"
